...
export default function GeneralForemanDashboard() {
  ...
  const renderCrewCard = ({ item }: { item: CrewStatus }) => (
    <TouchableOpacity style={styles.crewCard}>
      <View style={[styles.crewStatus, { backgroundColor: item.status === 'active' ? '#4CAF50' : '#9E9E9E' }]}>
        <Text style={styles.crewStatusText}>{item.status.toUpperCase()}</Text>
      </View>
      
      <View style={styles.crewInfo}>
        <Text style={styles.crewName}>{item.name}</Text>
        <Text style={styles.foremanName}>{item.foreman}</Text>
    ...
    <TouchableOpacity style={styles.jobCard}>
      ...
      <View style={styles.jobInfo}>
        ...
        <View style={styles.jobDetails}>
          <Text style={styles.jobDetailText}>Due: {item.dueDate.toLocaleDateString()}</Text>
          <Text style={styles.jobDetailText}>Est: ${item.estimatedCost.toLocaleString()}</Text>
        </View>
    ...
    <ScrollView style={styles.metricsContainer}>
      ...
      <View style={styles.chartContainer}>
        ...
        <View style={styles.chartPlaceholder}>
          <Text style={styles.chartPlaceholderText}>Chart Data Available</Text>
        </View>
      </View>
    </ScrollView>
  );
...
const MetricCard = ({ title, value, color }: any) => (
  <View style={styles.metricCard}>
    <View style={[styles.metricIconPlaceholder, { backgroundColor: color }]} />
